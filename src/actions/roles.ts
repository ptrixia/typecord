...
export async function updateRole(roleId: string, values: { name?: string; color?: string; hoist?: boolean; mentionable?: boolean; permissions?: string; position?: number }) {
  const role = await db.role.findUnique({
    ...
    select: { 
      ...
      color: true, 
      hoist: true, 
      mentionable: true, // <--- Adicionado aqui
      permissions: true, 
      position: true, 
      isDefault: true, 
      managed: true 
  ...
  if (values.color !== undefined) {
    if (!validColor(values.color)) throw new Error('Cor inválida.');
    data.color = values.color;
  } 
  if (values.hoist !== undefined) data.hoist = values.hoist; 
  if (values.mentionable !== undefined) data.mentionable = values.mentionable;
  if (values.permissions !== undefined) {
    const bits = normalizePermissions(values.permissions);
    if (ctx.topPosition !== Number.MAX_SAFE_INTEGER && (bits & Permissions.ADMINISTRATOR) !== 0n) throw new Error('Você não pode conceder Administrador.');
    data.permissions = bits.toString();
  }
  if (values.position !== undefined && ctx.topPosition !== Number.MAX_SAFE_INTEGER) {
    const p = Math.max(1, Math.min(ctx.topPosition - 1, Math.floor(values.position)));
    data.position = p;
  }
