...
export default function MembersSidebar({ members }: { members: any[] }) {
  ...
  return (
    <>
          ...
          {groupedMembers.map(({ role, members }) => (
            <section key={role.id} className="mb-5 last:mb-0">
              ...
              <div className="space-y-0.5">
                {members.map((member) => {
                  const user = member.user || member; // Fall caso venha direto no member ou dentro de .user
                  return (
                    <div
                      key={member.id}
                    ...
                    >
                      <div className="relative shrink-0">
                        ...
                        />
                        <span
                          className={`absolute bottom-[-1px] right-[-1px] h-2.5 w-2.5 rounded-full border-2 border-white dark:border-[#111214] ${getStatusColor(user?.status)}`}
                        />
                      </div>
                      <span
      ...
      {/* POPUP DE PERFIL */}
      {selectedMember && (() => {
        const user = selectedMember.user || selectedMember;
        return (
          <div
            ref={popupRef}
            ...
            <div className="relative px-4">
              <div className="absolute left-4 -top-12">
                <div className="relative">
                  <div className="rounded-full border-[6px] border-white dark:border-[#111214]">
                    <Avatar
                      avatarUrl={user.avatarUrl}
                      username={user.username}
                      globalName={user.globalName}
                  ...
                  <span className={`absolute bottom-1 right-1 h-5 w-5 rounded-full border-[4px] border-white dark:border-[#111214] ${getStatusColor(user.status)}`} />
                </div>
              </div>
              ...
              <div className="pt-12">
                ...
                <div className="text-sm text-stone-500 dark:text-zinc-400">@{user.username}</div>
              </div>
              ...
              <div className="pb-4 flex items-center gap-2">
                ...
                <button type="button" title="Copiar ID" onClick={() => navigator.clipboard.writeText(String(user.id))} className="flex h-10 w-10 items-center justify-center rounded-md bg-stone-200 text-stone-600 transition-colors hover:bg-stone-300 hover:text-stone-900 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 dark:hover:text-white">
